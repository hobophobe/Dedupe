...
# Updated 20101210 by Adam Dane; should handle manifest flags on (skin|locale)
# lines; NOT thoroughly tested to ensure proper behavior in all cases, but Works
# For Me.
if [ -f "chrome.manifest" ]; then
  ...
  sed -i -r s/^\(skin\|locale\)\(\\s+\\S*\\s+\\S*\\s+\)\(.*\\/\)\(.*\)$/\\1\\2jar:chrome\\/$APP_NAME\\.jar!\\/\\3\\4/ chrome.manifest
